...
export class GitHubIssueClient {
  ...
  async replaceLabels(issueNumber: number, labels: string[]): Promise<void> {
    const desired = [...new Set(labels.map((label) => label.trim()).filter(Boolean))];
    const path = this.repoPath(`issues/${issueNumber}/labels`);
    try {
      // GitHub: expects {"labels":[...]}.
      await this.req(path, { method: "PUT", body: JSON.stringify({ labels: desired }) });
      return;
    } catch (error) {
      // Some GitHub-compatible backends accept a raw array instead.
      const msg = String(error);
      if (msg.includes("HTTP 400") || msg.includes("HTTP 422")) {
        await this.req(path, { method: "PUT", body: JSON.stringify(desired) });
        return;
      }
      throw error;
    }
  ...
  async syncManagedLabels(issueNumber: number, desired: string[]): Promise<void> {
    ...
    await this.replaceLabels(issueNumber, [...unmanaged, ...desired]);
  }
